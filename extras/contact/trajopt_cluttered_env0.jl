...
let
    ...
    for i = 1:length(P_obs)
        ...
        dc.build_primitive!(vis, P_obs[i], Symbol(name); α = 1.0,color = mc.RGBA(normalize(abs.(randn(3)))..., 1.0))
        dc.update_pose!(vis[Symbol(name)],P_obs[i])
    end
    ...
    for k = 1:N
        mc.atframe(anim, k) do
            ...
            # r2 = [-r[2] , r[3],r[1]]
            # r2 = [-r[2]]
            # mc.settransform!(vis["/Cameras/default/rotated/<object>"],mc.Translation(r2) ∘ mc.LinearMap(diagm(ones(3))))
        end
    end
