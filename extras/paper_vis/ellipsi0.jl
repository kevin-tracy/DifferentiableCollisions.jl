...
E1 = Ellipsoid(SMatrix{3,3}(Diagonal([1,1/2,1/3] .^ 2)))
E2 = Ellipsoid(SMatrix{3,3}(Diagonal([1/2,1/1.2,1/2.4] .^ 2)))

c1 = [245, 155, 66]/255
c2 = [2,190,207]/255
build_primitive!(vis,E1,:e1;α = 1.0, color = mc.RGBA(c1..., 1.0))
E1.q = normalize((@SVector randn(4)))
E1.r = 2*(@SVector randn(3))
update_pose!(vis,E1,:e1)
build_primitive!(vis,E2,:e2;α = 1.0, color = mc.RGBA(c2..., 1.0))
E2.q = normalize((@SVector randn(4)))
E2.r = 3*(@SVector randn(3))
update_pose!(vis,E2,:e2)
...
α = α.value
@info α
build_primitive!(vis,E1,:e1_big;α = α, color = mc.RGBA(c1..., 0.4))
update_pose!(vis,E1,:e1_big)
build_primitive!(vis,E2,:e2_big;α = α, color = mc.RGBA(c2..., 0.4))
update_pose!(vis,E2,:e2_big)
