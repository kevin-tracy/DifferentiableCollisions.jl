...
trans_1 = 1.0
trans_2 = 0.5
cone = DCD.Cone(3.0,deg2rad(22))
cone.r = SA[-1,0,2.0]
cone.q = normalize((@SVector randn(4)))

polytope = DCD.Polytope(SMatrix{8,3}(A2),SVector{8}(b2))
polytope.q = normalize((@SVector randn(4)))
polytope.r = SA[0,1,2.0]

DCD.build_primitive!(vis, cone, :cone; α = 1.0,color = mc.RGBA(c2..., trans_2))
DCD.update_pose!(vis[:cone],cone)
DCD.build_primitive!(vis, polytope, :polytope; α = 1.0,color = mc.RGBA(c1..., trans_2))
...
for i = 1:length(αs)
    DCD.build_primitive!(vis, cone, cone_names[i]; α = αs[i],color = mc.RGBA(c2..., trans_1))
    DCD.update_pose!(vis[cone_names[i]],cone)
    DCD.build_primitive!(vis, polytope, polytope_names[i]; α = αs[i],color = mc.RGBA(c1..., trans_1))
...
#
sph_p1 = mc.HyperSphere(mc.Point(x_star...), 0.15)
