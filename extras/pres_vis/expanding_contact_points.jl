...
for k = 1:(2*length(αs) - 1)
    mc.atframe(anim, k) do
        if k <= length(αs)
            mc.setvisible!(vis[:p1], false)
            mc.setvisible!(vis[:p2], false)
            for i = 1:length(αs)
                mc.setvisible!(vis[cone_names[i]], false)
                mc.setvisible!(vis[polytope_names[i]], false)
            end
            ...
            idx = k - 2*(k -  length(αs))
            mc.setvisible!(vis[:p1], true)
            mc.setvisible!(vis[:p2], true)
            p1 = cone.r + (x_star - cone.r)/rαs[idx + 1]
            p2 = polytope.r + (x_star - polytope.r)/rαs[idx + 1]
            mc.settransform!(vis[:p1], mc.Translation(p1))
            mc.settransform!(vis[:p2], mc.Translation(p2))
