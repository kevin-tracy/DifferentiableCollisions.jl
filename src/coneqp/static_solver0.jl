...
# struct NT2{n_ort,n_soc,n_soc2,T}
#     W_ort::SVector{n_ort,T}
#     W_soc::SMatrix{n_soc,n_soc,T,n_soc2}
#     W_soc_fact::Cholesky{T, SMatrix{n_soc, n_soc, T, n_soc2}}
# end
#
# @inline function Base.:\(W1::NT2{n_ort,n_soc,n_soc2,T},g::SVector{r,T}) where {n_ort,n_soc,n_soc2,T,r}
#     idx_ort = SVector{n_ort}(1:n_ort)
#     idx_soc = SVector{n_soc}((n_ort + 1):(n_ort + n_soc))
#     [g[idx_ort] ./ W1.W_ort; W1.W_soc_fact\g[idx_soc]]
# end
...
# # end
# @inline @generated function Base.:\(W1::NT2{n_ort,n_soc,n_soc2,T},G::SMatrix{r,c,T,rc}) where {n_ort,n_soc,n_soc2,T,r,c,rc}
#     xi = [:(W1\G[:,$i]) for i = 1:c]
#     quote
#         hcat($(xi...))
#     end
# end
# @inline function Base.:*(W1::NT2{n_ort,n_soc,n_soc2,T}, b::SVector{n,T}) where {n_ort,n_soc,n,T,n_soc2}
#     idx_ort = SVector{n_ort}(1:n_ort)
#     idx_soc = SVector{n_soc}((n_ort + 1):(n_ort + n_soc))
#     [W1.W_ort .* b[idx_ort]; W1.W_soc*b[idx_soc]]
# end
#
# function nt_scalings(s::SVector{n,T}, z::SVector{n,T}, idx_ort::SVector{n_ort,Ti}, idx_soc::SVector{n_soc,Ti}) where {n,T,n_ort,n_soc,Ti}
#     idx_ort = SVector{n_ort}(1:n_ort)
#     idx_soc = SVector{n_soc}((n_ort + 1):(n_ort + n_soc))
#
#     # ort
...
#     z̄ = DCD.normalize_soc(z_soc)
#     s̄ = DCD.normalize_soc(s_soc)
...
#     η = (DCD.soc_quad_J(s_soc)/DCD.soc_quad_J(z_soc))^(1/4)
#     W_soc = η*W̄
#
#     W_ort, W_soc
# end
...
function solve_socp(c::SVector{nx,T},
    ...
    for main_iter = 1:20
        ...
        # W_ort, W_soc = nt_scalings(s,z,idx_ort,idx_soc)
        # W = NT2(W_ort,W_soc,cholesky(W_soc))
...
function tt()
    ...
    # solve_socp(c,G,h,idx_ort,idx_soc; verbose = false)
    # a =  @code_warntype solve_socp(c,G,h,idx_ort,idx_soc; verbose = false)
    # @show a
    # x = @SVector randn(5)
