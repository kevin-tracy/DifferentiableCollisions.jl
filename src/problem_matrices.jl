...
# solve for collision
function problem_matrices(capsule::Capsule{T},r::SVector{3,T1},q::SVector{4,T2}) where {T,T1,T2}
    n_Q_b = dcm_from_q(q)
    bx = n_Q_b*SA[1,0,0.0]
...
function problem_matrices(cone::Cone{T},r::SVector{3,T1},q::SVector{4,T2}) where {T,T1,T2}
...
function problem_matrices(prim::P) where {P<:AbstractPrimitive}
